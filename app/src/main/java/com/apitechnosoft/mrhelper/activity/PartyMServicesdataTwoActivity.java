...
public class PartyMServicesdataTwoActivity extends AppCompatActivity implements View.OnClickListener{
    Toolbar toolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_party_mservicesdata_two);
    ...
    public void chechPortaitAndLandSacpe() {
        if (CompatibilityUtility.isTablet(PartyMServicesdataTwoActivity.this)) {
    ...
    private void parsePartyData() {
        if (data != null) {
            ...
            if (partyMServicesdataTow != null) {
                sunheading = partyMServicesdata.getSubHeading();
                wheredo.setText(heading);
                subHeading.setText(sunheading);
                firstOption.setText(partyMServicesdataTow.getOption());
                secondOption.setText(partyMServicesdataTow.getOption2());
                thirdOption.setText(partyMServicesdataTow.getOption3());
                fourthOption.setText(partyMServicesdataTow.getOption4());
                fifthOption.setText(partyMServicesdataTow.getOption5());
            }
        }
    }
        ...
        switch (view.getId()) {
            ...
            case R.id.nextButtonLayout:
                if (isfirst || isSecond || isThree || isFour || isFive) {
                    ...
                    Utility.alertForErrorMessage("Please select anyone option!", PartyMServicesdataTwoActivity.this);
                }
                break;
        }
    }
    private void setValue(){
        ...
        editor.putString("Step2", selectedValue());
        editor.commit();
        if (!partyMServicesdataThree.getSno().equals("0") && partyMServicesdataThree.getOption() != null && !partyMServicesdataThree.getOption().equals("")) {
            Intent intent = new Intent(PartyMServicesdataTwoActivity.this, PartyMServicesdataThreeActivity.class);
            String option2Str = new Gson().toJson(partyMServicesdataThree);
            intent.putExtra("Option2", option2Str);
            intent.putExtra("heading", heading);
            startActivity(intent);
        }else {
            Intent intent = new Intent(PartyMServicesdataTwoActivity.this, AddressActivity.class);
    ...
    private String selectedValue() {
        ...
        if (partyMServicesdataTow != null) {
            if (isfirst) {
                selectvalue = partyMServicesdataTow.getOption();
            } else if (isSecond) {
                selectvalue = partyMServicesdataTow.getOption2();
            } else if (isThree) {
                selectvalue = partyMServicesdataTow.getOption3();
            } else if (isFour) {
                selectvalue = partyMServicesdataTow.getOption4();
            } else if (isFive) {
                selectvalue = partyMServicesdataTow.getOption5();
            }
        }
        return selectvalue;
    }
